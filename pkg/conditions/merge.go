...
func merge(conditions []localizedCondition, targetCondition crhelpertypes.ConditionType, options *mergeOptions) *crhelpertypes.Condition {
...
func (g conditionGroups) TrueGroup() *conditionGroup {
	return g.getByStatusAndSeverity(corev1.ConditionTrue, crhelpertypes.ConditionSeverityNone)
}
...
func (g conditionGroups) ErrorGroup() *conditionGroup {
	return g.getByStatusAndSeverity(corev1.ConditionFalse, crhelpertypes.ConditionSeverityError)
}
...
func (g conditionGroups) WarningGroup() *conditionGroup {
	return g.getByStatusAndSeverity(corev1.ConditionFalse, crhelpertypes.ConditionSeverityWarning)
}

func (g conditionGroups) getByStatusAndSeverity(status corev1.ConditionStatus, severity crhelpertypes.ConditionSeverity) *conditionGroup {
	if len(g) == 0 {
		return nil
	}
	for _, group := range g {
...
type conditionGroup struct {
	...
	severity   crhelpertypes.ConditionSeverity
...
func (g conditionGroup) mergePriority() int {
	...
	case corev1.ConditionFalse:
		switch g.severity {
		case crhelpertypes.ConditionSeverityError:
			return 0
		case crhelpertypes.ConditionSeverityWarning:
			return 1
		case crhelpertypes.ConditionSeverityInfo:
			return 2
		}
